# 자주 묻는 질문 (FAQ)

## 설치 및 실행

### Q. 설정파일(d2r.ini)은 어디에 있나요?

**환경설정 → 환경설정 읽기/저장하기 → 현재 설정을 파일로 저장**을 실행하면
프로그램과 같은 폴더에 `d2r.ini` 파일이 생성됩니다.

### Q. 악성코드로 인식합니다.

악성코드가 **아닙니다!**

파이썬으로 제작된 프로그램은 `pyinstaller` 같은 패키징 도구로 실행파일을 만듭니다.
이런 도구로 봇이나 악성코드가 많이 만들어지다 보니 백신에서 오탐지합니다.

**해결**: 백신 프로그램에서 예외 처리 후 사용

---

## 접속 문제

### Q. 접속이 되지 않습니다.

이 프로그램의 역할은 **클라이언트 실행까지**입니다.
이후 접속 문제는 계정 또는 배틀넷 서버 문제입니다.

**확인사항**:

1. **계정 정보 확인**: 사용자, 비밀번호가 정확한지
2. **CAPTCHA**: 이전 로그인 실패로 CAPTCHA 활성화 여부
3. **OTP**: 인증기 사용 계정은 명령줄 실행 불가
4. **서버 상태**: 점검, 업데이트, 접속자 과다
5. **클라이언트 버전**: 업데이트 필요 여부

### Q. 친구목록이 뜨지 않습니다.

명령줄 실행 시 친구목록 로딩에 시간이 걸립니다.
배틀넷 런처는 미리 로그인되어 있어서 빠르지만,
명령줄 실행은 순서상 늦게 로딩됩니다.

잠시 기다리면 표시됩니다.

---

## 게임 기능

### Q. 토큰은 어떻게 만드나요?

**자동 발급 (권장)**:

1. 사용자, 비밀번호 입력
2. 토큰 입력창 옆 아이콘 클릭
3. OTP 사용 시 30초 이내 인증

**수동 발급**:

1. 크롬 시크릿 창 열기
2. `https://us.battle.net/login/en/?externalChallenge=login&app=OSI` 접속
3. 로그인 후 에러 페이지의 `ST=` 뒤 값 복사

### Q. 게임 이름은 어떻게 만들어지나요?

`게임 제목 + 일련번호` 형식

- 게임 제목: `MAG`
- 일련번호: `1`
- 자릿수: 2

→ 결과: `MAG01`, `MAG02`, ... `MAG99`

자릿수 변경: 설정파일의 `digit_size = 3`

### Q. 게임 생성/참가 시 입력이 안됩니다.

**스크립트 속도 조정**이 필요합니다.

설정파일(d2r.ini):

```ini
script_delay_time = 0.1   # 0 ~ 0.2 사이로 조정
```

0.025 단위로 조금씩 늘려보세요.

---

## 스크립트

### Q. 스크립트 시간 조정 방법이 있나요?

**전체 속도 조정**:

```ini
script_delay_time = 0.1
```

**개별 지연 추가**:

```ini
create = sleep 1,        # 1초 대기 후 시작
    click 850 55,
    # ...
```

### Q. 클릭 위치는 어떻게 얻나요?

1. 설정파일에서 `resize_window = True` 설정
2. 창모드에서 최소화 (1280x720)
3. AutoHotkey의 **Windows Spy** 같은 도구로 좌표 확인

**비율 좌표** 사용:

```ini
click 0.5 0.5    # 정중앙
```

### Q. 자동에서 레거시 모드를 생략하고 싶습니다.

설정파일의 `auto` 스크립트에서 레거시 관련 부분 삭제:

```ini
auto = loop {all_games},
    execute exit,
    end,
    loop {master_game},
    execute create,
    end,
    loop {slave_games},
    execute join,
    end,
    sleep 4.5,
    activate {master_game}   # 레거시 부분 삭제됨
```

---

## 단축키

### Q. 단축키가 먹통이 됩니다.

한/영키가 분리된 103/106키 키보드 문제입니다.

```ini
hangul_keyboard = True
```

설정 후 프로그램 재시작

---

## 기타

### Q. 설정이 초기화됩니다.

D2R은 마지막 저장/종료한 설정이 모든 클라이언트에 적용됩니다.

**해결**: 클라이언트별 설정파일 지정

1. `%userprofile%\Saved Games\Diablo II Resurrected\Settings.json` 복사
2. 원하는 이름으로 저장 (예: `Settings_Low.json`)
3. 계정 설정에서 설정파일 선택

### Q. 매크로가 동작하지 않습니다.

D2R 클라이언트가 관리자 권한으로 실행되므로,
AHK, 로지텍 GHub, PowerToys 등도 **관리자 권한**으로 실행해야 합니다.

### Q. D2R 클라이언트가 자꾸 죽습니다.

클라이언트를 **최소화하면 메모리 누수** 버그가 있습니다.
사용하지 않더라도 화면 뒤쪽에 띄워 놓으세요.

### Q. 여러 개 실행 시 꺼져버립니다.

**원인**:

- PC/그래픽카드 메모리 부족
- 실행 간격이 너무 빠름

**해결**:

- 하단 패널의 실행 간격을 4초 이상으로 증가

### Q. 폴더 복사를 해야 하나요?

- **명령줄 실행**: 복사 불필요
- **배틀넷 런처**: 계정당 하나씩 필요

용량이 부족하면 **정션(Junction) 폴더** 사용:

**도구 → 링크폴더 만들기**

또는 수동으로:

```cmd
mklink /j "C:\...\Diablo II Resurrected2" "C:\...\Diablo II Resurrected"
```

### Q. 모드 적용은 어떻게 하나요?

1. `D2R 설치폴더\mods\모드명` 폴더에 모드 파일 배치
2. 계정 설정에서 **런처**를 `d2r.exe`로 선택
3. **모드** 콤보박스에서 모드 선택
4. 저장 후 실행

!!! note "참고"
    Launcher.exe는 실행 방식이 달라 모드 선택이 안됩니다.

### Q. 나가기 버튼 클릭이 이상한 위치입니다.

사용하는 모드(스킨)에서 버튼 위치가 변경된 경우입니다.

**해결**:

1. 기본 스킨 사용
2. 또는 설정파일의 나가기 스크립트에서 클릭 좌표 수정

**키보드로 나가기**:

```ini
exit = activate,
    send esc,
    send up,
    send enter
```
